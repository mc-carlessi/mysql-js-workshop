...
5. Library functions can be invoked only within the library or stored routine into which their containing library is imported.
  To import a library into your function, use 'USING'.  The keyword 'AS ...? is option, but can be used to simplify the code reading

  **![orange-dot](./images/orange-square.jpg) mysqlsh>**
  ```js
  <copy>
  CREATE FUNCTION myfunc1(n INTEGER) RETURNS INTEGER DETERMINISTIC
  LANGUAGE JAVASCRIPT
...
6. Now execute the function that uses the library
...
7. JavaScript syntax is checked at library creation time.  
